...
{\phi\vdash{x}:{C}\\\vdash{C.f}:{T}\\\phi\vdash{y}:{T}\\\acc(x,f) \impliedby \phi\\x{\:\neq\:}\vnull \impliedby \phi}
...
{\phi\vdash{y}:{C}\\\phi\vdash{x}:{T_r}\\\phi\vdash{z'}:{T_p}\\y{\:\neq\:}\vnull \impliedby \phi\\\phi{\:\implies\:}( \phi_p * \phi_r )\\ \mmethod(C,m) = T_r~m(T_p~z)~\requires{\phi_{pre}};~\ensures{\phi_{post}};~\{\_\} \\ \phi_p = \phi_{pre}[y,z'/\xthis,z] \\ \phi_q = \phi_{post}[y,z',x/\xthis,z,\xresult] }
...
{x\not\in \dom(\phi_1)\\ \phi_2 = \phi_1{\:*\:}x{\::\:}T }
