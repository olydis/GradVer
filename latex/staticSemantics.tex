...
{{\phi} \implies {{\acc({x}.{f})} * {{({{x}} \neq {{\vnull}})} * {\phi'}}}\\{\emptyset} \sfrmphi {\phi'}\\{x} \not \in {FV(\phi')}\\{\phi} \vdash {{x}} : {{C}}\\{\phi} \vdash {{y}} : {T}\\\vdash {C}.{f} : {T}}
{\hoare {\phi} {{x}.{f} := {y}} {{{x} : {{C}}} * {{\acc({x}.{f})} * {{({{x}} \neq {{\vnull}})} * {{({{{x}}.{f}} = {{y}})} * {\phi'}}}}}}
\end{mathpar}

\begin{mathpar}
\inferrule* [Right=HVarAssign]
{{\phi} \implies {\phi'}\\{\emptyset} \sfrmphi {\phi'}\\{x} \not \in {FV(\phi')}\\{\phi} \vdash {{x}} : {T}\\{\phi} \vdash {e} : {T}\\{\staticFP {\phi'}} \sfrme {e}}
