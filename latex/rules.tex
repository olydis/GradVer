...
\subsubsection{HFieldAssign (deterministic alternative)}
...
\subsubsection{GHFieldAssign (deterministic alternative)}
...
{
\vdash C.f : T \\\\
\grad{\phi_1} \grad{\implies}_{\phi_m} {(x : C) * (y : T) * \acc(x.f)} \\
...
\item for the gradual rules to work, the \textbf{w/o}-operator must be defined with minimal information loss (i.e. not syntactically - or maybe after some transitive-hull operation?)
\end{itemize}


\subsubsection{GHFieldAssign (implication alternative)}
