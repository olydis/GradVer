...
Trying to “fix” GHSec statically turns out to be hard - and moves our focus on dynamic semantics. 
...
\begin{description}
	...
	\begin{mathpar}
		...
		{
			\Gamma \hoare {\hat{\phi_{pre}}} {s} {\hat{\phi_{post}}}\\
			\hat{\phi_{pre}} \implies \text{goal}(s)\\
			env \vdash \hat{\phi_{pre}}
		}
		{ s(env) \vdash \hat{\phi_{post}}}
	\end{mathpar}
	...
	\begin{align*}
	&\GTappVER(\grad{\phi_{pre}}, \grad{\phi_{post}})\\
&\implies\\
...
&\implies\\ % because \grad{imp} is somewhat precise!?
	&\Gamma ~\vdash~ s ~\wedge~\\
...
&\implies\\
	...
	env ~\vdash~ \hat{\phi_{pre}}\\
&\implies\\
	\exists \hat{\phi_{pre}} \in \grad{\phi_{pre}}, \hat{\phi_{post}} \in \grad{\phi_{post}} : 
	&\TappVER(\hat{\phi_{pre}}, \hat{\phi_{post}})
	\end{align*}
	
	Note that it implicitly ensures the consistent transitivity of the gradual version of $\hat{\phi} \implies \hat{\phi_{pre}} \implies \text{goal}(s)$ ($\hat{\phi}$ was simplified away earlier).
	...
	Bottom line, instead of introducing evidence we end up with an additional premise $env \vdash \withqm{\text{goal}(s)}$ which can easily be checked at runtime.
	% Note: optimization.
\end{description}
