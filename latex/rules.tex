...
\subsubsection{Liftable Composition in general}
Motivation: While composition works out great in most static Hoare rules it does not in HSec. Subtelty, etc., ...
...
\begin{description}
	...
	Let's pretend we had instead given rules of constructing environments and see how to derive \textit{direct} runtime semantics from that.
	In this setting, there would have to be a rule analogous to Tapp:
	\begin{mathpar}
		\inferrule* [Right=TappVER]
		{
			\Gamma \vdash s : \phi_{pre} \rightarrow \phi_{post}\\
			\vdash env : \phi\\
			\phi <: \phi_{pre}
		}
		{ \vdash s(env) : \phi_{post} }
	\end{mathpar}
	Leveraging our syntax:
	\begin{mathpar}
		\inferrule* [Right=TappVER]
		{
			\Gamma \hoare {\phi_{pre}} {s} {\phi_{post}}\\
			env \vdash \phi\\
			\phi \implies \phi_{pre}
		}
		{ s(env) \vdash \phi_{post}}
	\end{mathpar}
	...
	\begin{mathpar}
		\inferrule* [Right=$\grad{\text{T}}$appVER]
		{
			...
			env \vdash \grad{\phi_{pre}}
		}
		{ s(env) \vdash \grad{\phi_{post}} }
	\end{mathpar}
	Note that the only reasonable definition of $env \vdash \grad{\phi}$ would be to see if the static part of $\grad{\phi}$ evaluates under $env$ - there is no further meaning to \qm as soon as it is evaluated.
	
	Just as it is the case with $\grad{\text{T}}$app, this rule is imprecise, requiring additional measures to ensure 
	
	Under which circumstances might this rule fail at runtime, i.e. when is $s(env)$ stuck?
	From soundness of the static hoare rules, we know that it can only be stuck when the Hoare-rules didn't apply.
	The Hoare-rules turn out to be partial functions, being undefined only if $s$ itself is malformed (you could split that part of the static semantics apart from everything involving $\phi$!) or if the precondition violates some implication.
	
	Let's call the thing to be implied $\text{goal}(s)$ and define it as $\true$ whenever the Hoare-rule actually doesn't require any implication to hold. We can rewrite TappVER accordingly:
	\begin{mathpar}
		\inferrule* [Right=TappVER]
		{
			\Gamma \hoare {\phi_{pre}} {s} {\phi_{post}}\\
			\phi_{pre} \implies \text{goal}(s)\\
			\vdash env : \phi\\
			\phi \implies \phi_{pre}
		}
		{\vdash s(env) : \phi_{post}}
