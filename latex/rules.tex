...
\subsection{Type ($\texttt{staticType}_{\phi}(e) = T$)}
\begin{align*}
 &\texttt{staticType}_{\phi}(v_T)	&&= T
\\ &\texttt{staticType}_{\phi}(x)	&&= T  ~~\text{ where } \phi \implies (x : T)
\\ &\texttt{staticType}_{\phi}(e.f)	&&= \texttt{fieldType}(C, f)  ~~\text{ where } \texttt{staticType}_{\phi}(e) = C
\end{align*}
